...
function checkUser($tbl_staff, $tbl_department, $txtusername, $txtpassword, $from_ldap = 'Y') {
    ...
    if (($txtpassword != "") && ($txtusername != "")) {
        if ($from_ldap != '') {
            $str_add = " and s.from_ldap='" . $from_ldap . "' ";
        }
        if ($from_ldap == 'N') {
...
// 27-AUG-2012 by Sirivimol T.
function getDepartment2($dbname, $tbl_department, $tbl_employee, $dep_id = "", $dep_name = "", $party_id = '', $search_status = '') {
    $arr_return = "";
    $str_add = "";
    if ($dep_id != "") {
    ...
    if ($dep_name != "") {
        $str_add .= " d.`dep_name` = '$dep_name' and";
    } // end if    
    if ($party_id != "") {
        $str_add .= " d.`party_id` = '$party_id' and";
    } // end if
    if ($search_status != "") {
        $str_add .= " d.`search_status` = '$search_status' and";
    } // end if
    if ($str_add != "") {
        $str_add = "where " . substr($str_add, 0, strlen($str_add) - 3);
    }
    $sql = "select d.`dep_id`, d.`dep_name`, d.`party_id` from `$dbname`.`$tbl_department` AS d
INNER JOIN `$dbname`.`$tbl_employee` AS t ON d.dep_id = t.dep_id
 $str_add order by d.`dep_name`, d.`dep_id`";
    //     echo $sql . "<br>";
    $result = mysql_query($sql);
...
function drawing2($limit, $show_console = FALSE) {
    // check if exist data
    $sql1 = "select *  from " . TBL_EMPLOYEE . " where prize_no is not null order by prize_no desc limit " . $limit;
...
//    echo "</pre>";
    if ($result1) {
        $old_prize_no = $result1[0]['prize_no'];
        $old_party_id = $result1[0]['party_id'];
        $old_dep_id = $result1[0]['dep_id'];
    }// end if

    $prize_no = $old_prize_no + 1;

    // echo ' cnt = ' . $prize_no . "<br>";
    // if no data
    $sql = 'SELECT * FROM ' . TBL_EMPLOYEE . " where give_up = 'N' and prize_no is null and party_id <>'$old_party_id' and dep_id <> '$old_dep_id' ORDER BY RAND() LIMIT " . $limit;
    // echo $sql;
    $result = select($sql);
    if ($show_console) {
        echo "<pre>";
    ...
    if ($result) {
        ...
        $sql2 = 'SELECT * FROM ' . TBL_EMPLOYEE . " where give_up = 'N' and prize_no is null  ORDER BY RAND() LIMIT " . $limit;
        // echo $sql;
        $result2 = select($sql2);
        if ($show_console) {
            echo "<pre>";
        ...
        if ($result2) {
            foreach ($result2 as $line => $arr_data) {
...
function showLucky2($max_row) {
    $sql2 = "select * from " . TBL_EMPLOYEE . " where (prize_no is not null)  order by prize_no desc limit 0,$max_row";
    $result_show = select($sql2);
    return $result_show;
}
