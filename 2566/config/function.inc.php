...
function copytable($table2, $table1) {
...
function getEmployee($tbl_employee, $price_status = '', $emp_search_name = "", $dep_search_id = '', $give_up = 'N') {
    ...
    $str_add = "";
    if ($price_status == "Y") {
        $str_add .= " d.`prize` is not null and";
    } // end if        
    if ($emp_search_name != "") {
	...
	//$sql = "SELECT  d.* FROM $tbl_employee as d $str_add order by d.prize_no, d.dep_name, d.emp_id, d.emp_firstname, d.emp_surname ";
	//$sql = "SELECT  d.*, b.*, c.* FROM $tbl_employee as d LEFT JOIN tbl_prize_name as b ON d.prize = b.prize_id LEFT JOIN " . TBL_DEPARTMENT . " as c ON d.dep_id = c.dep_id $str_add order by d.prize, d.prize_seq, c.dep_name, d.emp_id, d.emp_firstname, d.emp_surname ";
	//$sql = 'SELECT  d.*, b.*, c.* FROM $tbl_employee as d LEFT JOIN ' . TBL_PRIZE_NAME . ' as b ON d.prize = b.prize_id LEFT JOIN ' . TBL_DEPARTMENT . ' as c ON d.dep_id = c.dep_id $str_add order by d.prize, d.prize_seq, d.dep_name, d.emp_id, d.emp_firstname, d.emp_surname';
	$sql = "SELECT  d.* FROM $tbl_employee as d  $str_add  order by case when d.prize_no is NULL then 1 else 0 end, d.prize_no ASC, d.prize_seq ASC, d.dep_name, d.emp_id, d.emp_firstname, d.emp_surname ";
    //echo $sql . "<br>";
    $result = mysql_query($sql);
    //echo $result . "<br>";
    return $result;
...
function clearDataAll($show_console = FALSE) {
    $newdata = array(
        "prize_no" => 'null',
		"prize_seq" => 'null',
		"prize" => 'null',
    );
	//$copytable = copytable(TBL_EMPLOYEE . "_copy", TBL_EMPLOYEE);
    $result = update(TBL_EMPLOYEE, $newdata, "prize_no IS NOT NULL OR prize_no <> '' OR prize_seq IS NOT NULL OR prize_seq <> '' OR prize IS NOT NULL OR prize <> ''");
    $sql = "DELETE FROM tbl_prize_name";
...
function drawing($limit, $prize, $show_console = FALSE) {
...
//        echo ' cnt = ' . $count . "<br>";
//    }// end if
	$sql2 = 'select count(prize_no) as countno2 from ' . TBL_EMPLOYEE . " Where prize_no is not null";
    $sql3 = "SELECT COUNT(id) as countno3 FROM `tbl_prize_name` WHERE 1";
	$result2 = mysql_query($sql2);
    $result3 = mysql_query($sql3);
    
	if ($result2) {
        $rowmax = mysql_fetch_array($result2);
        $countno2 = $rowmax["countno2"];
	}

    if ($result3) {
        $rowmax3 = mysql_fetch_array($result3);
        $prize_no = $rowmax3["countno3"] + 1;
        $timestamp = date("Y-m-d H:i:s");
        $data = array(
            "prize_id" => $prize_no,
            "prize_amount" => $limit,
            "last_update_by" => $timestamp
        );
        $insert_result = insert(DATABASE . "." . TBL_PRIZE_NAME, $data);
    }
    ...
    if ($count == 0) {
        // if no data
        $sql = 'SELECT * FROM ' . TBL_EMPLOYEE . ' where give_up = \'N\' and prize_no is null and prize is null ORDER BY RAND() LIMIT ' . $limit;
        $result = select($sql);
        foreach ($result as $line => $arr_data) {
			$prize = ($line + 1) + $countno2;
		...
		//update(TBL_EMPLOYEE, $newdata2, "prize_no is not null ");
		//$sql = 'UPDATE ' . TBL_EMPLOYEE . ' SET prize_seq = prize_no WHERE prize_seq is NULL';
    	//$result = mysql_query($sql);
		//$strSQL = "UPDATE ". TBL_EMPLOYEE . "SET prize_seq = prize_no WHERE prize_seq is NULL";
		//mysql_query($strSQL);	
}

function showLucky($max_row = 5) {
	$sql = 'select prize_amount as mm from ' . TBL_PRIZE;
    $result = mysql_query($sql);
    if ($result) {
			$rowlimit = mysql_fetch_array($result);
            $limit = $rowlimit['mm'];
    }
	//echo $limit;
	 $sql1 = 'select count(prize_no) as countno1 from ' . TBL_EMPLOYEE . " Where prize_no is not null";
     $sql3 = "SELECT COUNT(id) as countno3 FROM `tbl_prize_name` WHERE 1";
	 $result1 = mysql_query($sql1);
     $result3 = mysql_query($sql3);
	     
	if ($result1) {
        $rowmax1 = mysql_fetch_array($result1);
        //echo $countno1 = $rowmax1["countno1"];
	}

    if ($result3) {
        $rowmax3 = mysql_fetch_array($result3);
        $prize_no = $rowmax3["countno3"];
    }

	$countno1 = $countno1 - $limit;
    //$sql2 = "select a.*, b.* from " . TBL_EMPLOYEE . " as a LEFT JOIN " . TBL_DEPARTMENT . " as b ON a.dep_id = b.dep_id where (a.prize_no is not null and a.prize_no > $countno1) order by a.prize_no";
	$sql2 = "select a.*, b.* from " . TBL_EMPLOYEE . " as a LEFT JOIN " . TBL_DEPARTMENT . " as b ON a.dep_id = b.dep_id where a.prize_no = $prize_no order by a.prize_seq ASC";
	//$sql = "SELECT  d.*, b.*, c.* FROM $tbl_employee as d LEFT JOIN tbl_prize_name as b ON d.prize = b.prize_id LEFT JOIN " . TBL_DEPARTMENT . " as c ON d.dep_id = c.dep_id $str_add order by d.prize, d.prize_seq, c.dep_name, d.emp_id, d.emp_firstname, d.emp_surname ";
    ...
    if ($result_show != '') {
        foreach ($result_show as $line => $row) {
            ...
            //$str .= '<tr><td class="tdlucky">' . ($line + 1) . '.</td><td class="tdlucky">' . $prefix . " " . $firstname . "  " . $surname . '<br><span class="tdlucky2">' . $dep_name . "</span></td></tr>";
			$str .= '<tr><td class="tdlucky">' . "ลำดับที่ " . $prize_seq . " : ".' </td><td class="tdlucky">' . $prefix . " " . $firstname . "  " . $surname . '<br><span class="tdlucky2">' . $dep_name . "</span></td></tr>";
            if ((($line + 1) % $max_row) == 0) {
                $status_show = true;
            } // end if
