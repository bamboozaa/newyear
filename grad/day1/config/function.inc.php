...
function drawing($limit, $show_console = FALSE) {
    ...
    // check if exist data
    $sql1 = "SELECT party_id FROM " . TBL_EMPLOYEE . " GROUP BY party_id order by RAND() LIMIT 0, $limit";
...
//    echo "</pre>";
    do {
        ...
        if (isset($result1)) {
            foreach ($result1 as $line => $arr_temp) {
                $party_id = $arr_temp['party_id'];

                $sql = 'SELECT * FROM ' . TBL_EMPLOYEE . " where give_up = 'N'  and party_id = '$party_id' ORDER BY RAND() LIMIT 1 ";
   //             echo $sql . "<br>";
                $result = select($sql);
                if ($show_console) {
                    echo "<pre>";
