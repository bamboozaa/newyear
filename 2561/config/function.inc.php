...
//	$result2 = select($sql2);
    if ($count == 0) {
        // if no data
        $sql = 'SELECT * FROM ' . TBL_EMPLOYEE . ' where give_up = \'N\' and prize_no is null ORDER BY RAND() LIMIT ' . $limit;
...
function showLucky($max_row = 5) {
	...
	$sql2 = "select * from " . TBL_EMPLOYEE . " where (prize_no is not null and prize_no > $countno1) order by prize_no";
    ...
    if ($result_show != '') {
        foreach ($result_show as $line => $row) {
			...
			$str .= '<tr><td class="tdlucky">' . "รางวัลที่ " . $prize_no2 . " : ".' </td><td class="tdlucky">' . $prefix . " " . $firstname . "  " . $surname . '<br><span class="tdlucky2">' . $dep_name . "</span></td></tr>";
            if ((($line + 1) % $max_row) == 0) {
                $status_show = true;
            } // end if
