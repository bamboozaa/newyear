...
function getEmployee($tbl_employee, $price_status = '', $emp_search_name = "", $dep_search_id = '', $give_up = 'N') {
	...
	//$sql = "SELECT  d.* FROM $tbl_employee as d  $str_add  order by d.dep_name, d.prize_no, d.emp_id, d.emp_firstname, d.emp_surname ";
    // echo $sql . "<br>";
    $result = mysql_query($sql);
    //   echo $result . "<br>";
    return $result;
...
function showLucky($max_row = 5) {
    ...
    if ($result_show != '') {
        foreach ($result_show as $line => $row) {
			...
			$str .= '<tr><td class="tdlucky">' . "ลำดับที่ " . $prize_no2 . " : ".' </td><td class="tdlucky">' . $prefix . " " . $firstname . "  " . $surname . '<br><span class="tdlucky2">' . $dep_name . "</span></td></tr>";
            if ((($line + 1) % $max_row) == 0) {
                $status_show = true;
            } // end if
