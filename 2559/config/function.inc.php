...
function checkUser($tbl_staff, $tbl_department, $txtusername, $txtpassword) {
    $return_status = false;
    if (($txtpassword != "") && ($txtusername != "")) {
        $sql = "SELECT s.emp_username, s.emp_firstname, s.emp_surname, s.emp_email, s.dep_id, s.from_ldap,  s.last_login_date, s.last_update_date, d.dep_name , s.admin_status
FROM " . DATABASE . ".$tbl_staff AS s 
Inner Join " . DATABASE . ".$tbl_department AS d ON d.dep_id = s.dep_id WHERE lower(s.emp_username) = '" . strtolower($txtusername) . "' ";
//        echo $sql . "<br>";
...
function getEmployee($tbl_employee, $price_status = '', $emp_search_name = "", $dep_search_id = '') {
    ...
    $sql = "SELECT  d.* FROM $tbl_employee as d  $str_add  order by d.dep_name, d.prize_no, d.emp_firstname, d.emp_surname ";
//     echo $sql . "<br>";
    $result = mysql_query($sql);
    //   echo $result . "<br>";
    return $result;
